...
export default function UpiLiteTapPaymentScreen() {
  ...
  useEffect(() => {
    ...
    loop.start();
    const id = setTimeout(() => void complete('UPI Lite Tap'), 2600);
    return () => {
      loop.stop();
      ring.setValue(0);
      clearTimeout(id);
  ...
  return (
    <View style={styles.screen}>
      <View style={styles.top}>
        <TrustChip label="NFC tap · on-device · Never leaves the store" tone="light" icon="🔒" />
      </View>
      ...
      <View style={styles.center}>
        ...
        <Text style={styles.sub}>Reading your UPI Lite app to authorise this payment…</Text>
        <Text style={styles.amount}>₹{total.toLocaleString('en-IN')}</Text>
        {error ? <Text style={styles.error}>{error}</Text> : null}
      </View>

      <View style={styles.bottom}>
        <Pressable onPress={() => void complete('UPI Lite Tap · UPI PIN')} style={styles.skip}>
