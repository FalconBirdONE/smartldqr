...
const { BasketModule, TransactionModule } = NativeModules as {
  BasketModule: {
    getBasketItems(): Promise<string>;
    getBasketTotal(): Promise<number>;
    clearBasket(): Promise<boolean>;
  };
  TransactionModule: {
...
export default function CheckoutScreen() {
  ...
  const loadBasket = useCallback(async () => {
    setLoading(true);
    try {
      const [json, basketTotal] = await Promise.all([
        BasketModule.getBasketItems(),
        BasketModule.getBasketTotal(),
      ]);
  ...
  const handleSimulateScan = async () => {
    ...
    setError(null);
    try {
      const transactionId = await TransactionModule.logTransaction({
        total_amount: total,
        payment_method: 'SIMULATED_QR',
        basket_snapshot: JSON.stringify(items),
        item_count: items.reduce((sum, item) => sum + item.quantity, 0),
      });
      await BasketModule.clearBasket();
      router.replace({
